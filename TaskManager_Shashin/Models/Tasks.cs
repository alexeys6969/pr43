...
namespace TaskManager_Shashin.Models
{
    public class Tasks : Notification
    {
        ...
        public DateTime DateExecute
        {
            ...
            set
            {
                if (value < DateTime.Now)
                    MessageBox.Show("Дата выполнения не может быть меньше текущей даты.",
                        "Некорректный ввод значения.");
                else
                {
                    dateExecute = value;
        ...
        [Schema.NotMapped]
        public TaskManager_Shashin.Classes.RealyCommand OnEdit
        {
            get
            {
                return new TaskManager_Shashin.Classes.RealyCommand(obj =>
                {
                    IsEnable = !IsEnable;
                    if (!IsEnable)
        ...
        [Schema.NotMapped]
        public TaskManager_Shashin.Classes.RealyCommand OnDelete
        {
            get
            {
                return new TaskManager_Shashin.Classes.RealyCommand(obj =>
                {
                    if (MessageBox.Show("Вы уверены, что хотите удалить эту задачу?", "Подтверждение удаления", MessageBoxButton.YesNo) == MessageBoxResult.Yes)
                    {
        ...
        [Schema.NotMapped]
        public TaskManager_Shashin.Classes.RealyCommand OnDone
        {
            get
            {
                return new TaskManager_Shashin.Classes.RealyCommand(obj =>
